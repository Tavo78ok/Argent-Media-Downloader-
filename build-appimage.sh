...
BIN_NAME="media_downloader"
VERSION="${1:-3.5}"
ARCH="$(uname -m)"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SRC_PY="${SCRIPT_DIR}/media_downloader.py"
...
# --------------------------------------------------------------------------
[ -f "$SRC_PY" ] || die "No se encontró '${SRC_PY}'. Coloca este script junto a media_downloader.py."
...
exec "${PYTHON_BIN}" "${HERE}/usr/bin/media_downloader.py" "$@"
APPRUN
